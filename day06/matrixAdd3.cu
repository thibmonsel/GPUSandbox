...
approach. Let's use a 2D grid and 2d block for this matrix addition.
*/
...
void matrixAddOnHost(float *A, float *B, float *C, const int nx, const int ny)
{
...
int main(void)
{
    ...
    matrixAddOnHost(h_A, h_B, h_C, nx, ny);
    ...
    matrixAddonDevice<<<grid, block>>>(d_A, d_B, d_C, nx, ny);
    ...
    if (millisecondsTotal > 0)
    {
        ...
        double totalBytesTransferred = nBytes;
