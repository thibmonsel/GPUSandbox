...
__global__ void reduceNeighbored(int *g_idata, int *g_odata, unsigned int n)
{
    ...
    unsigned int block_start_idx = blockIdx.x * blockDim.x; // Global start index for this block's data segment

    for (unsigned int s = 1; s < blockDim.x; s *= 2)
    {
        // Check if the current thread is an "adder" thread for this stride
        if ((tid % (2 * s)) == 0)
        {
            unsigned int write_global_idx = block_start_idx + tid;
            unsigned int read_global_idx = block_start_idx + tid + s;
        ...
        // Synchronize all threads in the block in order to avoid write and read conflicts
        __syncthreads();
    }

    // After the loop, for each block, the element at g_idata[block_start_idx]
    // (i.e., where tid=0 of the block would write) contains the reduced sum
    // for all valid elements processed by this block.

    // Thread 0 of each block writes this block's reduced sum to g_odata.
    if (tid == 0)
    {
        // Ensure that this block is actually responsible for some valid data.
...
void test_reduction()
{
    ...
    for (size_t N : problem_sizes)
    {
        ...
        reduceNeighbored<<<gridDim, blockDim>>>(d_idata, d_odata, N);
