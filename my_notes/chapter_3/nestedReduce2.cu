...
// Fixed kernel - removed the cudaDeviceSynchronize() call
                                   ...
                                   unsigned int isize)
{
    ...
    // Only launch the nested kernel if we're thread 0
    if (tid == 0)
    {
        gpuRecursiveReduce<<<1, istride>>>(idata, odata, istride);
        // Remove the cudaDeviceSynchronize() call here
    }
...
int main(int argc, char **argv)
{
    ...
    // initialize the array - fixed to keep random values
    for (int i = 0; i < size; i++)
    {
        h_idata[i] = (int)(rand() & 0xFF);
        // Removed the line that overwrites with 1
    }
    ...
    free(h_idata);
    free(h_odata);
    free(tmp); // Added free for tmp to avoid memory leak
